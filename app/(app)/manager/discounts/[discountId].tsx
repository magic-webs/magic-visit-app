...
import { revealDiscountOtp, AuthBridgeError, type RevealDiscountOtpResult } from "@/lib/auth-bridge";
...
export default function ManagerDiscountDetailScreen() {
  ...
  const [revealed, setRevealed] = useState<RevealDiscountOtpResult | null>(null);
  ...
  async function handleReveal() {
    ...
    setRevealing(true);
    try {
      const result = await revealDiscountOtp(request.id);
      setRevealed(result);
    } catch (e) {
      setRevealError(e instanceof AuthBridgeError ? e.message : "Couldn't reveal the code.");
    } finally {
      setRevealing(false);
    }
  }
        ...
        <Card className="flex-row items-center gap-3">
          ...
          <View className="flex-1">
            ...
            <AppText className="font-sans text-sm text-[#6b7280]">
              {formatDiscount(request.discountType, request.discountValue)} discount · requested by{" "}
          ...
          revealed ? (
            <>
              <Card className="items-center gap-2 py-6">
                <AppText className="font-sans text-sm text-[#6b7280]">One-Time Code</AppText>
                <AppText className="font-sans-bold text-4xl tracking-widest text-[#1c1c1e]">{revealed.otp}</AppText>
              </Card>
              <InfoBanner
                icon={KeyRound}
                title="Tell this code to the accountant only"
                description="Say it aloud in person — don't forward it by text or leave it visible on screen."
              />
            </>
          ) : (
            <>
              {revealError && <AppText className="font-sans text-sm text-status-notInterested">{revealError}</AppText>}
              <Button onPress={handleReveal} loading={revealing}>
                Reveal Code
              </Button>
            </>
          )
